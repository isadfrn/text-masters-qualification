...
Neste capítulo, detalha-se a solução proposta para responder às questões de pesquisa definidas na introdução: o desenvolvimento de um \textbf{\textit{Framework} de Diretrizes para Inclusão e Retenção}. A proposta consiste em correlacionar os dados quantitativos de desempenho e engajamento do Programa \textit{Hackers} do Bem com a análise qualitativa da percepção das participantes, fundamentando as diretrizes em evidências empíricas.
...
Com base no \autoref{c_trabalhos_relacionados}, o \textit{framework} estrutura-se preliminarmente em três eixos estratégicos, que serão refinados a partir da análise dos dados do estudo de caso:
...
\begin{itemize}
    \item \textbf{Transição $T_1$ (Nivelamento $\rightarrow$ Básico):} Análise de desistência espontânea em fase assíncrona, na qual não há barreiras de entrada competitivas.
    ...
    \item \textbf{Transição $T_4$ (Especializado $\rightarrow$ Residência):} A etapa final envolve a escolha de trilhas (Red Team, Blue Team, GRC e Forense). A análise focará na segregação vertical, verificando se há concentração feminina em trilhas de gestão em detrimento de trilhas técnicas ofensivas/defensivas.
\end{itemize}

\section{Avaliação da Proposta} \label{s_validacao_proposta}
Considerando que a implementação longitudinal das diretrizes em um programa massivo como o \textit{Hackers} do Bem excede o escopo temporal deste mestrado, a avaliação da efetividade (redução real da evasão) não é viável nesta etapa. Portanto, a avaliação do \textit{Framework} foca na avaliação de conteúdo e na avaliação de percepção, utilizando uma abordagem de triangulação com dois grupos distintos: especialistas e participantes.
\subsection{Avalidação com Especialistas: Viabilidade e Pertinência} \label{ss_validacao_especialistas}
O objetivo desta etapa é verificar se as diretrizes propostas são tecnicamente aplicáveis em um ambiente de larga escala e se são pedagogicamente adequadas para o público-alvo. O instrumento de coleta será um questionário estruturado, submetido a um painel de especialistas (docentes de cibersegurança, gestores de programas massivos e pesquisadores de gênero em TI).
Os critérios de avaliação serão: \begin{itemize} \item \textbf{Pertinência:} A diretriz ataca o problema real de evasão diagnosticado na fase anterior? \item \textbf{Viabilidade de Escala:} A diretriz pode ser implementada para 30.000 alunos sem inviabilizar a operação técnica ou financeira do programa? (Baseado nas limitações de escala citadas em \citeonline{Tshekiso2025}). \item \textbf{Clareza:} A diretriz é compreensível para os gestores e desenvolvedores que deverão implementá-la? \end{itemize}
\subsection{Avaliação com Participantes: Percepção de Valor e Intenção} \label{ss_validacao_participantes}
Esta etapa visa avaliar a aderência das diretrizes às necessidades das alunas. Será aplicado um questionário a uma amostra de participantes do programa (ativas e evadidas), apresentando cenários hipotéticos baseados nas diretrizes do \textit{framework} (ex: \enquote{Se o curso oferecesse mentorias em grupo em vez de ranking individual, isso aumentaria sua vontade de continuar?}).
Os construtos avaliados baseiam-se no modelo ARCS (Atenção, Relevância, Confiança e Satisfação) descrito por \citeonline{AlZoubi2024} e nas métricas de autoeficácia de \citeonline{Casey2023}:
\begin{enumerate} \item \textbf{Relevância Percebida:} Avaliar se a contextualização narrativa proposta no \textit{framework} aumenta a percepção de utilidade do conteúdo para a carreira da aluna. \item \textbf{Segurança Psicológica:} Avaliar se as diretrizes de avaliação inclusiva reduzem a ansiedade gerada pela competição (validando os achados de \citeonline{Hogan2025}). \item \textbf{Intenção de Permanência:} Medir a probabilidade declarada de a aluna persistir no curso caso as diretrizes fossem implementadas. \end{enumerate}
Desta forma, não se avalia se a evasão diminuiu (o que é impossível agora), mas se a \textbf{proposta tem potencial teórico e aceitação do público} para gerar esse resultado no futuro.

\section{Definição dos Instrumentos de Coleta}
...
Nesta dissertação, o GQM é aplicado para estruturar os dois instrumentos de coleta primária: o questionário de percepção voltado às participantes do Programa \textit{Hackers} do Bem e o questionário de avaliação voltado aos especialistas.
...
\subsection{Instrumento 2: Avaliação com Especialistas}
\label{ss_gqm_especialistas}

Após a estruturação do \textit{Framework} de Diretrizes, este será submetido a um painel de especialistas para avaliação de conteúdo e viabilidade.

\begin{itemize}
    \item \textbf{Objetivo de Medição (Goal):} Avaliar o \textit{Framework} de Diretrizes proposto, com o propósito de avaliar sua viabilidade técnica e potencial de impacto, sob o ponto de vista de especialistas em educação em cibersegurança e gestores de programas, no contexto da implementação em larga escala.
\end{itemize}
...
Para as alunas do Programa \textit{Hackers} do Bem, a amostragem será por conveniência, divulgada nos canais oficiais de comunicação do programa (E-mail), visando atingir tanto alunas ativas quanto evadidas. Para os especialistas, será utilizada uma amostragem intencional, selecionando perfis com experiência comprovada em gestão de programas de capacitação em cibersegurança ou pesquisa em gênero em \ac{STEM}.
...
\subsection{Síntese da Proposta}
\label{s_sintese_proposta}

Para responder a este desafio, esta dissertação propõe o desenvolvimento e a avalição do \textbf{\textit{Framework} de Diretrizes para Inclusão e Retenção}. Diferentemente de soluções que focam apenas na atração (\textit{marketing}) ou em ações pontuais, a proposta busca intervir na arquitetura pedagógica e nas regras de negócio dos programas de formação.

A abordagem metodológica mista, detalhada no Capítulo 4, foi desenhada para ir além da percepção superficial. Ao cruzar os dados quantitativos de evasão e desempenho no funil do programa (mineração de dados) com a análise qualitativa da autoeficácia das participantes (\textit{survey}), a pesquisa permitirá isolar variáveis críticas. Busca-se responder se a evasão feminina ocorre por falta de competência técnica ou se é induzida por mecanismos de \textit{design}, como a pressão de rankings públicos e a ausência de narrativas contextualizadas, que afetam desproporcionalmente a confiança deste grupo.
...
\begin{itemize}
    \item \textbf{Contribuição Científica:} O fornecimento de evidências empíricas sobre o impacto da gamificação competitiva na retenção de mulheres em \ac{MOOCs} de cibersegurança. A pesquisa pretende preencher a lacuna identificada por \citeonline{SelmanHousein2025} sobre a escassez de métricas rigorosas de avaliação de intervenções de gênero.
