...
\section{Validação da Proposta} \label{s_validacao_proposta}
Considerando que a implementação longitudinal das diretrizes em um programa massivo como o \textit{Hackers} do Bem excede o escopo temporal deste mestrado, a validação da efetividade (redução real da evasão) não é viável nesta etapa. Portanto, a validação do \textit{Framework} foca na validação de conteúdo e na validação de percepção, utilizando uma abordagem de triangulação com dois grupos distintos: especialistas e participantes.
\subsection{Validação com Especialistas: Viabilidade e Pertinência} \label{ss_validacao_especialistas}
...
\subsection{Validação com Participantes: Percepção de Valor e Intenção} \label{ss_validacao_participantes}
