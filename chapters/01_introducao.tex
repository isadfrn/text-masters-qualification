...
\begin{itemize}
    ...
    \item \textbf{QP3:} Em que medida a customização de trilhas de aprendizagem e a implementação de mecanismos colaborativos podem influenciar a percepção de autoeficácia e a intenção de permanência das estudantes, em programas como o \textit{Hackers} do Bem?
\end{itemize}
...
\begin{itemize}
    ...
    \item Elaborar o \textbf{\textit{Framework} de Diretrizes para Inclusão e Retenção em Escala}, fundamentado nos diagnósticos quantitativos e qualitativos.
    ...
    \item Avaliar a \textbf{percepção de valor} e o impacto potencial das diretrizes na intenção de permanência das alunas do Programa \textit{Hackers} do Bem, por meio de questionários de avaliação.
\end{itemize}
...
Quanto à sua natureza, esta pesquisa classifica-se como aplicada, pois objetiva gerar conhecimentos para aplicação prática, dirigidos à solução de um problema específico: a mitigação da evasão feminina em programas de formação em escala. Do ponto de vista dos seus objetivos, é exploratória, visando proporcionar maior familiaridade com o problema do \textit{leaky pipeline} no contexto de cursos gamificados massivos, uma área onde as correlações entre mecânicas de jogo e gênero ainda demandam aprofundamento empírico \cite{SelmanHousein2025}.
...
\begin{enumerate}
    ...
    \item \textbf{Concepção e Modelagem do \textit{Framework} (Artefato):} Nesta etapa, será realizada a construção teórica do \textbf{\textit{Framework} de Diretrizes para Inclusão e Retenção em Escala}. A modelagem do artefato não se limitará à síntese da literatura, mas consistirá na adaptação e extensão de modelos de engajamento consolidados para o contexto específico da formação massiva em cibersegurança. A base conceitual será o modelo \ac{IDLE}, citado por \citeonline{Casey2023}, que estrutura o aprendizado através de missões investigativas e narrativas imersivas.

    \item \textbf{Validação da Proposta:} A validação do \textit{framework} se dará por meio de instrumentos de coleta de dados definidos através da metodologia \ac{GQM}. Aplicação de questionário para docentes e gestores de programas de cibersegurança visando avaliar a \enquote{Viabilidade de Implementação em Escala} e a \enquote{Pertinência Pedagógica} das diretrizes. Bem como, aplicação de \textit{survey} com alunas do Programa \textit{Hackers} do Bem (ativas e evadidas). As questões serão fundamentadas no Modelo \ac{ARCS} de \citeonline{Keller1987}, conforme aplicado por \citeonline{AlThebyan2025}, para mensurar se as diretrizes propostas teriam impacto positivo na motivação e na \enquote{Intenção de Permanência} das estudantes.
\end{enumerate}
