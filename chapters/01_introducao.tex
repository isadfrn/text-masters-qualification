...
Apesar da crescente demanda por profissionais de cibersegurança e dos esforços globais para diversificar a força de trabalho, as mulheres permanecem sub-representadas, ocupando apenas cerca de 25\% dos postos de trabalho globais na área \cite{Kshetri2022, ISC2_2023, ISC2_2024, ISC2_2025, ISACA2025, ITU2024}. A literatura aponta para um fenômeno persistente de \enquote{vazamento de talentos} (\textit{leaky pipeline}), onde as taxas de evasão femininas superam as masculinas à medida que o nível técnico e a competitividade aumentam \cite{Pitman2022}. Este fenômeno não é apenas uma questão de recrutamento, mas de retenção, influenciada por barreiras estruturais, culturais e pedagógicas \cite{BothaBadenhorst2023, SelmanHousein2025}.
...
\begin{itemize}
    ...
    \item \textbf{QP3:} Em que medida a customização de trilhas de aprendizagem e a implementação de mecanismos colaborativos podem influenciar a percepção de autoeficácia e a intenção de permanência das estudantes em programas de capacitação em cibersegurança, como o \textit{Hackers} do Bem?
\end{itemize}
...
\begin{itemize}
    ...
    \item Avaliar a correlação entre os mecanismos de gamificação (\ac{XP}, Ranking), outras estratégias de engajamento e o desempenho das participantes do programa, por meio da análise dos dados do programa \textit{Hackers} do Bem.
    
    \item Elaborar o \textbf{\textit{Framework} de Diretrizes para Inclusão e Retenção}, fundamentado nos diagnósticos quantitativos e qualitativos.
    
    \item Avaliar a \textbf{viabilidade técnica} e a \textbf{pertinência pedagógica} das diretrizes propostas junto a um painel de especialistas. 
    
    \item Avaliar a \textbf{percepção de valor} e o impacto potencial das diretrizes na intenção de permanência das alunas do Programa \textit{Hackers} do Bem, por meio de questionários de avaliação com as participantes do programa.
\end{itemize}

\section{Metodologia}
\label{s_metodologia}

Esta seção apresenta a metodologia de pesquisa adotada e o procedimento metodológico aplicado para realização deste trabalho.

\subsection{Metodologia da Pesquisa}
\label{ss_metodologia_pesquisa}

Este trabalho adota como abordagem metodológica o método hipotético-dedutivo. A investigação a partir da lacuna de conhecimento identificada na literatura sobre o fenômeno do \enquote{vazamento de talentos} (\textit{leaky pipeline}) em cibersegurança \cite{Pitman2022} e a hipótese central é de que a eficácia de programas massivos depende não apenas do conteúdo técnico, mas da integração de estratégias de suporte pedagógico e social. A literatura indica que mecanismos de gamificação competitiva, quando não contrabalançados por abordagens inclusivas, podem gerar barreiras de permanência \cite{Hogan2025, Horcher2021}. Tais hipóteses serão avaliadas a partir da análise dos documentos, dados de participação e entrevistas com mulheres, participantes do Programa \textit{Hackers} do Bem, objeto deste estudo.

Contudo, estudos recentes demonstram que a retenção feminina é significativamente ampliada quando são adotadas estratégias complementares, tais como: a contextualização narrativa dos desafios, conectando-os a problemas sociais reais \cite{Costa2023, Casey2023}; a valorização de \textit{soft skills} e competências colaborativas em detrimento da execução solitária \cite{Benson2025, Tshekiso2025}; e a estruturação de redes de mentoria e suporte comunitário (\textit{wrap-around support}) para mitigar o isolamento em ambientes virtuais \cite{SelmanHousein2025, Musuva2025}. Desta forma, esta pesquisa investiga como a articulação destas estratégias, no contexto do Programa \textit{Hackers} do Bem, pode influenciar positivamente os indicadores de retenção de talentos femininos.
...
Quanto à sua natureza, esta pesquisa classifica-se como aplicada, pois objetiva gerar conhecimentos para aplicação prática, dirigidos à solução de um problema específico: a mitigação da evasão feminina em programas de formação em escala. Do ponto de vista dos seus objetivos, é exploratória, visando proporcionar maior familiaridade com o problema do \enquote{vazamento de talentos} (\textit{leaky pipeline}) no contexto de cursos gamificados massivos, uma área onde as correlações entre mecânicas de jogo e gênero ainda demandam aprofundamento empírico \cite{SelmanHousein2025}.

No que tange à abordagem, adota-se um método misto, combinando a análise quantitativa de dados educacionais (para identificar padrões de evasão) com a análise qualitativa da percepção das participantes (para compreender as motivações subjacentes), permitindo a triangulação necessária para a construção do \textit{Framework} de Diretrizes proposto.
...
\begin{enumerate}
    ...
    \item \textbf{Análise de trabalhos relacionados:} Leitura e análise dos trabalhos selecionados pela \ac{RSL}, a fim de mapear as estratégias de retenção e barreiras de gênero documentadas na literatura.
    ...
    \item \textbf{Análise de Dados:} Processamento estatístico para cálculo de taxas de evasão e análise quantitativa do funil de progressão do programa, bem como análise qualitativa conduzida por meio de entrevistas com as as participantes.

    \item \textbf{Concepção e Modelagem do \textit{Framework}:} Nesta etapa, será realizada a construção teórica do \textbf{\textit{Framework} de Diretrizes para Inclusão e Retenção}. A modelagem do artefato não se limitará à síntese da literatura, mas consistirá na adaptação e extensão de modelos de engajamento consolidados para o contexto específico da formação massiva em cibersegurança. A base conceitual será o modelo \ac{IDLE}, citado por \citeonline{Casey2023}, que estrutura o aprendizado através de missões investigativas e narrativas imersivas.

    \item \textbf{Avaliação da Proposta:} A avaliação do \textit{framework} se dará por meio de instrumentos de coleta de dados definidos através da metodologia \ac{GQM}. Aplicação de questionário para profissionais nas áreas de cibersegurança, educação e diversidade, visando avaliar a \enquote{Viabilidade de Implementação em Escala} e a \enquote{Pertinência Pedagógica} das diretrizes. Bem como, aplicação de \textit{survey} com alunas do Programa \textit{Hackers} do Bem (ativas e evadidas). As questões serão fundamentadas no Modelo \ac{ARCS} de \citeonline{Keller1987}, conforme aplicado por \citeonline{AlThebyan2025}, para mensurar se as diretrizes propostas teriam impacto positivo na motivação e na \enquote{Intenção de Permanência} das estudantes.
\end{enumerate}
